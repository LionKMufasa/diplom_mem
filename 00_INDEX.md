# Project Memory Index

This is the external project memory for the CoppeliaSim palletizing/dynamics and VKR deliverables work.

Canonical project root:

- `C:\Users\egork\Desktop\coppelia_dpilom`

Read in this order after context compaction or in a new session:

1. [[01_CURRENT_STATE]] - current scene, scripts, known behavior, open questions.
2. [[03_TASKS]] - active tasks and next concrete steps.
3. [[02_DECISIONS]] - durable decisions and reasons.
4. [[04_USER_PREFERENCES]] - stable user preferences and working style.
5. Latest daily log in [[logs/2026-05-05]] or newer.
6. [[memory_workflow]] - how to use this memory and resume after compaction.
7. [[project_structure]] - canonical folder layout.
8. [[docs/project_deliverables_plan]] - VKR/NIRS deliverables, source materials, and recommended chat split.
9. [[docs/vkr_rpz/working_state]] - current VKR RPZ document state and writing workflow.
10. [[docs/vkr_rpz/introduction_compact]] - compact source used for the 70-page RPZ compression pass.
11. [[docs/vkr_rpz/chapter1_predproject_compact]] - compact chapter 1 source used for the 70-page RPZ compression pass.
12. [[docs/vkr_rpz/chapter2_conceptual_compact]] - compact chapter 2 source used for the 70-page RPZ compression pass.
13. [[docs/vkr_rpz/chapter3_tz_draft]] - source Markdown used to fill VKR RPZ chapter 3.
14. [[docs/vkr_rpz/chapter3_tz_gost_strict]] - strict ГОСТ 34.602-89 chapter 3 source used in the latest DOCX correction pass.
15. [[docs/vkr_rpz/chapter4_technical_design]] - source Markdown used to fill VKR RPZ chapter 4.
16. [[docs/vkr_rpz/chapter5_working_design]] - source Markdown used to fill VKR RPZ chapter 5.
17. [[docs/vkr_rpz/chapter6_approbation_effectiveness]] - source Markdown used to fill VKR RPZ chapter 6.
18. [[docs/vkr_rpz/current_written_summary_and_practice_plan]] - concise summary of written RPZ and practical implementation plan.
19. [[docs/vkr_rpz/final_insert_delete_map]] - concrete map for deleting/replacing RPZ figure/table placeholders after practical PAK work.
20. [[docs/vkr_rpz/pdf_review_2026-05-28]] - review of the user's saved VKR RPZ PDF, including table-caption fixes and topic-alignment notes.
21. [[docs/vkr_rpz/appendices_2026-05-28]] - appendix structure added to the VKR RPZ and main-text references to appendices.
22. [[docs/vkr_rpz/audit_findings_2026-05-29]] - latest audit findings, user decisions, and correction priorities.
23. [[docs/vkr_rpz/software_architecture_revision_2026-06-02]] - accepted plan for chapter 2 software-choice/architecture restructuring and chapter 5 digital-model preparation addition.
24. [[docs/vkr_practice/pak_pipeline_plan]] - working PAK pipeline, acceptance criteria, and RPZ output package.
25. [[docs/vkr_practice/coppeliasim_motion_manual_analysis]] - local CoppeliaSim manual/example analysis for the closed-chain robot motion blocker.
26. [[docs/vkr_practice/data_pipeline_state]] - implemented file data pipeline and latest smoke-test outputs.
27. [[docs/vkr_practice/pak_runtime_runbook]] - how to run the PAK demo, including another-computer setup.
28. [[docs/vkr_rpz/literature_citation_notes]] - literature/PDF citation map for VKR RPZ.
29. [[docs/vkr_rpz/chapter2_conceptual_draft]] - earlier expanded Markdown draft used to fill VKR RPZ chapter 2.
30. [[docs/nirs8_rpz/nirs8_work_plan]] - NIRS-8 RPZ focus, source document, and next steps.
31. [[docs/nirs8_rpz/current_docx_state]] - current NIRS-8 DOCX structure and fill status.
32. [[docs/nirs8_rpz/nirs8_full_draft_source]] - generated source mirror for the filled NIRS-8 DOCX.
33. [[docs/nirs8_rpz/nirs8_report_outline]] - planned NIRS-8 report structure.
34. [[docs/nirs8_rpz/nirs8_literature_selection]] - selected literature for NIRS-8 with download status.

Main entities:

- Final VKR scene name for RPZ text: `C:\Users\egork\Desktop\coppelia_dpilom\scenes\vkr_scena.ttt`
- Main cycle script: [[final_scene_palletizing_cycle]]
- VKR RPZ working document: `C:\Users\egork\Desktop\coppelia_dpilom\ВКР\ВКР 2026 Миронов Егор Максимович.docx`
- VKR RPZ working note: [[docs/vkr_rpz/working_state]]
- VKR RPZ compact introduction source: [[docs/vkr_rpz/introduction_compact]]
- VKR RPZ compact chapter 1 source: [[docs/vkr_rpz/chapter1_predproject_compact]]
- VKR RPZ compact chapter 2 source: [[docs/vkr_rpz/chapter2_conceptual_compact]]
- VKR RPZ chapter 3 source: [[docs/vkr_rpz/chapter3_tz_draft]]
- VKR RPZ strict chapter 3 source: [[docs/vkr_rpz/chapter3_tz_gost_strict]]
- VKR RPZ chapter 4 source: [[docs/vkr_rpz/chapter4_technical_design]]
- VKR RPZ chapter 5 source: [[docs/vkr_rpz/chapter5_working_design]]
- VKR RPZ chapter 6 source: [[docs/vkr_rpz/chapter6_approbation_effectiveness]]
- VKR RPZ summary and practical plan: [[docs/vkr_rpz/current_written_summary_and_practice_plan]]
- VKR RPZ final insert/delete map: [[docs/vkr_rpz/final_insert_delete_map]]
- VKR RPZ latest PDF review: [[docs/vkr_rpz/pdf_review_2026-05-28]]
- VKR RPZ appendices pass: [[docs/vkr_rpz/appendices_2026-05-28]]
- VKR RPZ latest audit findings: [[docs/vkr_rpz/audit_findings_2026-05-29]]
- VKR RPZ software/architecture revision plan: [[docs/vkr_rpz/software_architecture_revision_2026-06-02]]
- VKR practical PAK pipeline: [[docs/vkr_practice/pak_pipeline_plan]]
- CoppeliaSim motion manual/example analysis: [[docs/vkr_practice/coppeliasim_motion_manual_analysis]]
- VKR file data pipeline state: [[docs/vkr_practice/data_pipeline_state]]
- VKR PAK runtime runbook: [[docs/vkr_practice/pak_runtime_runbook]]
- VKR RPZ citation note: [[docs/vkr_rpz/literature_citation_notes]]
- VKR RPZ chapter 2 source draft: [[docs/vkr_rpz/chapter2_conceptual_draft]]
- NIRS-8 working document: `C:\Users\egork\Desktop\coppelia_dpilom\ВКР\НИРС(8сем)\НИРС 2026 Миронов Егор Максимович.docx`
- NIRS-8 planning note: [[docs/nirs8_rpz/nirs8_work_plan]]
- NIRS-8 current DOCX state: [[docs/nirs8_rpz/current_docx_state]]
- NIRS-8 full draft source mirror: [[docs/nirs8_rpz/nirs8_full_draft_source]]
- NIRS-8 report outline: [[docs/nirs8_rpz/nirs8_report_outline]]
- NIRS-8 literature selection: [[docs/nirs8_rpz/nirs8_literature_selection]]
- Memory usage guide: [[memory_workflow]]
- Folder layout: [[project_structure]]
- Deliverables plan: [[docs/project_deliverables_plan]]
- Old temporary memory location: `C:\Program Files\CoppeliaRobotics`

Update rules:

- Update [[01_CURRENT_STATE]] and [[03_TASKS]] after significant work.
- Update [[02_DECISIONS]] when a choice changes the architecture, simulation assumptions, report scope, or deliverable strategy.
- Add concise daily entries under `logs/YYYY-MM-DD.md`.
