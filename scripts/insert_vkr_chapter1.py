...
def apply_table_widths(table, rows: list[list[str]]) -> None:
    ...
    if col_count == 2:
        ...
        widths = [1.9, 2.35, 2.55]
    elif col_count == 4:
        widths = [1.55, 1.75, 1.75, 1.75]
    else:
...
def iter_content_blocks(markdown: str):
    ...
    # Drop the auxiliary insertion checklist from the working RPZ body.
    stop = next((idx for idx, line in enumerate(lines) if line.strip() == "## Список рекомендуемых вставок для главы 1"), len(lines))
    lines = lines[:stop]
...
def replace_chapter(docx_path: Path, draft_path: Path) -> None:
    doc = Document(str(docx_path))

    start_idx = find_index(doc, "Предпроектное обследование")
    end_idx = find_index(doc, "Концептуальное проектирование")

    start = doc.paragraphs[start_idx]
    clear_paragraph(start)
    start.add_run("Предпроектное обследование")
    ...
    for kind, value in iter_content_blocks(markdown):
        if kind == "paragraph":
            ...
            rows = value
            if rows:
                current = insert_table_after(current, rows)
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("Usage: insert_vkr_chapter1.py <docx> <chapter1_markdown>")
    replace_chapter(Path(sys.argv[1]), Path(sys.argv[2]))
    print(f"Inserted chapter 1 into {sys.argv[1]}")
