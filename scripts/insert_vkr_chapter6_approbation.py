...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("Usage: insert_vkr_chapter6_approbation.py <docx> <chapter6.md>")

    docx_path = Path(sys.argv[1])
    chapter6_path = Path(sys.argv[2])
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup = docx_path.with_name(
        f"{docx_path.stem}.backup_before_chapter6_approbation_{timestamp}{docx_path.suffix}"
    )
    ...
    doc.paragraphs[find_index(doc, "Апробация и оценка эффективности системы")].paragraph_format.page_break_before = True
    formula_changes = improve_formulas(doc)
    doc.save(str(docx_path))
    set_update_fields_on_open(docx_path)

    print(f"Inserted chapter 6 into {docx_path}")
